...
3. Проставить неразрывные пробелы (`alt`+`пробел` для OS X, ??? для Windows) для случаев:
  - для значений физических величин после числового значения и до единицы измерения (например, 1 кВт)
    - в Word это можно сделать с помощью подстановочных знаков (регулярных выражений)
      ...
      - В поле "Заменить" вбить регулярное выражение `\1{неразрывный пробел}\2` (в выражении заменить {неразрывный пробел} на сам неразрывный пробел прям в поле "Заменить")
